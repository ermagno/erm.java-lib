package it.erm.graphics.swing.items.list;

import it.erm.graphics.EList;
import it.erm.lib.crud.dbo.DBTable;

import java.awt.*;
import java.util.ArrayList;
import java.util.List;

import javax.swing.*;
import javax.swing.table.*;

public class ESList extends JScrollPane implements EList {

	private JTable  jtable; 
	private boolean searchAllFields = false;
	
	public ESList(DBTable table, boolean searchAllFields, boolean singleChoice) {
		super(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		jtable = new JTable(new ESTableModel(table, searchAllFields));
		jtable.createDefaultColumnsFromModel();
		if( singleChoice ) {
			jtable.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		} else {
			jtable.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		}
		TableColumnModel columnModel = jtable.getColumnModel();
		for( int i=0; i<columnModel.getColumnCount(); i++ ) {
		    TableColumn column = columnModel.getColumn(i);
            column.setHeaderRenderer(new DefaultTableCellRenderer() {
                @Override
                public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                    JLabel l = (JLabel) super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                    l.setHorizontalAlignment(JLabel.HORIZONTAL);
                    l.setBackground(Color.LIGHT_GRAY);
                    Font f = getFont();
                    l.setFont(new Font(f.getName(), Font.BOLD, f.getSize()+2));
                    return l;
                }
            });
		}
        setViewportView(jtable);
        setLayout(new ScrollPaneLayout());
        FontMetrics fm = new FontMetrics(jtable.getFont()) {};
        jtable.setRowHeight(fm.getHeight()+15);
        jtable.setIntercellSpacing(new Dimension(15, 0)); 
		this.searchAllFields = searchAllFields;
	}
	
	public boolean isSingleSelection() {
		return jtable.getSelectionModel().getSelectionMode() == ListSelectionModel.SINGLE_SELECTION;
	}
	
	public JTable getJTable() {
		return jtable;
	}
	
	public ESTableModel getTableModel() {
		return (ESTableModel) jtable.getModel();
	}
	
	@Override
	public List<Object> getRecords() {
		return getTableModel().getData();
	}

	public List<DBTable> getSelecteItems() {
		int selectedRowCount = getJTable().getSelectedRowCount();
		ArrayList<DBTable> selectedItems = new ArrayList<DBTable>(selectedRowCount);
		int[] selectedRows = getJTable().getSelectedRows();
		for( int index: selectedRows ) {
			selectedItems.add((DBTable) getRecords().get(index));
		}
		return selectedItems;
	}

	public void update() {
		getTableModel().fireTableDataChanged();
	}
	
}
